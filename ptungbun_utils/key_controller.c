...
/*   Updated: 2024/01/21 17:46:57 by pnopjira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_forward_or_back(t_main *m, int key)
{
	...
	ft_bzero(vp->bgimg->addr, WINDOW_WIDTH * \
	WINDOW_HEIGHT * vp->bgimg->bpp / 8);
	mlx_clear_window(vp->mlx, vp->win);
	cub3d_render(m, m->viewport);
}

void	turn_left_or_right(t_main *m, int key, t_player *p)
{
	t_ray		*ray;
	int			i;
	...
	ft_bzero(m->viewport->bgimg->addr, WINDOW_WIDTH * \
	WINDOW_HEIGHT * m->viewport->bgimg->bpp / 8);
...
int	bottons(int keycode, t_main *main)
{
	...
	keycode == KEY_D || keycode == KEY_RIGHT)
		turn_left_or_right(main, keycode, main->player);
